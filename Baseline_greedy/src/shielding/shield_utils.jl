...
struct ShieldUtils{P,Q}
    ...
    target_locations::Array{Location,1}  # Added target locations
    ϕ_sequence_set::P
...
function ShieldUtils(exp_details, pomdp_details, wheelbase)
    ...
    return ShieldUtils(
                ...
                exp_details.target_locations,  # Added target locations
                ϕ_sequence_set,
                safe_actions,
...
function vehicle_can_reach_goal(m, vehicle)
    ...
    #########
    current_goal = get_map_target(m.exp_details)  # Get current MAP goal
    ...
    while(num_steps <= max_num_steps)
        ...
        if(is_within_range(vehicle_center_x,vehicle_center_y, current_goal.x, current_goal.y, 
