...
function POMPDPlanningDetails(obj)
    return POMPDPlanningDetails(
        ...
        obj.target_transition_noise  # Added target transition noise
    )
end
...
function ExperimentDetails(obj)
    return ExperimentDetails(
        ...
        generate_environment(obj.env_length, obj.env_breadth, obj.obstacles),
        # Added target-related fields
        obj.target_locations,
        TargetBelief(obj.initial_target_belief),
        TargetState(obj.target_locations[1].x, obj.target_locations[1].y)
    )
end
