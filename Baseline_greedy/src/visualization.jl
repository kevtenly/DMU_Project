...
#=
Added "BG" (Believed Goal) marker showing MAP estimate

Added "TG" (True Goal) red star marker

Maintained original goal visualization as belief target

Added translucent circles showing probability distribution over goals

Alpha transparency scaled with belief probability
=#
...
function get_plot(env,vehicle,vehicle_params,humans,humans_params,sensor_data,nearby_humans,time_value,exp_details)
    ...
    ######## Plot Vehicle's Believed Goal (MAP estimate)
    map_goal = get_map_target(exp_details)
    Plots.annotate!(map_goal.x, map_goal.y, text("BG", :darkgreen, :right, 10))
    plot!(circleShape(map_goal.x, map_goal.y, exp_details.radius_around_vehicle_goal), 
        lw=0.5, linecolor=:black, fillalpha=0.2, c=:darkgreen, seriestype=[:shape])

    ######## Plot True Goal
    true_goal = exp_details.true_target
    Plots.annotate!(true_goal.x, true_goal.y, text("TG", :red, :left, 10))
    scatter!([true_goal.x], [true_goal.y], color=:red, markershape=:star5, markersize=8)
...
function get_plot(env, vehicle, vehicle_body, vehicle_params, nearby_humans, sensor_data, time_value, exp_details, vehicle_traj)
    ...
    ######### Plot Vehicle's Believed Goal (MAP estimate)
    map_goal = get_map_target(exp_details)
    Plots.annotate!(snapshot, map_goal.x, map_goal.y, text("BG", :darkgreen, :center, 10))
    plot!(snapshot, circleShape(map_goal.x, map_goal.y, exp_details.radius_around_vehicle_goal),
        color=:darkgreen, fillalpha=0.2, linecolor=:black, linewidth=2.0, seriestype=[:shape])

    ######## Plot True Goal
    true_goal = exp_details.true_target
    Plots.annotate!(snapshot, true_goal.x, true_goal.y, text("TG", :red, :center, 10))
    scatter!(snapshot, [true_goal.x], [true_goal.y], color=:red, markershape=:star5, markersize=10)
    ...
    plot!(snapshot, vehicle_traj_x_path, vehicle_traj_y_path,
        linez=vehicle_velocity_list, clim=(0,velocity_lim),
        linewidth=1,label="", colorbar=true, colorbar_title="Velocity [m/s]")
    plot!(snapshot, vehicle_traj_x_path, vehicle_traj_y_path,
...
function observe(output, exp_details, time_value, vehicle_executed_trajectory)
    ...
    # Create the base plot with environment elements
    p = get_plot(e, v, vb, vp, nbh, sd, time_value, exp_details, vehicle_executed_trajectory)
    

    
    # Mark all potential target locations
    for (i, target) in enumerate(exp_details.target_locations)
        is_true_target = isapprox(target.x, exp_details.true_target.x, atol=0.1) && 
                        isapprox(target.y, exp_details.true_target.y, atol=0.1)
        
        if is_true_target
            scatter!(p, [target.x], [target.y], color=:red, markershape=:star5, 
                    markersize=10, label="True Goal")
        else
            scatter!(p, [target.x], [target.y], color=:purple, markershape=:diamond, 
                    markersize=8, label=i==2 ? "Potential Goals" : "")
        end
    end

    
    
    # Add MAP goal visualization
    map_goal = get_map_target(exp_details)
    scatter!(p, [map_goal.x], [map_goal.y], color=:blue, markershape=:xcross, 
            markersize=10, label="MAP Goal")

    # Plot expected path if available
    # if(hasfield(typeof(vp), :controls_sequence))
    #     expected_path = output.vehicle_expected_trajectory[time_value]
    #     plot!(p, expected_path[1], expected_path[2], line=(3,:dot,:blue))
    # end
