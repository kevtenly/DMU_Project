...
using LinearAlgebra
using StaticArrays
# using .POMDPTypes: TargetState, StateLimitedSpacePOMDP, LimitedSpacePOMDP
...
#=
Modified reward functions to use true target location
=#
function vehicle_goal_reached_reward(s::TreeSearchScenarioParameters, goal_reward::Float64)
    if is_terminal_state(s, s.true_target)
        return goal_reward
    else
        return 0.0
    end
