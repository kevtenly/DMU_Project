    ...
    while(!stop_simulation!(current_sim_obj,current_time_value,exp_details,output))
        ...
        # If it is not, then take observation from the environment and plan for the next action
            ...
            pomdp_solver = DESPOTSolver(
                                ...
                                tree_in_info=true,
                                default_action=(m, b, ex) -> get_default_action(m, b, ex),
                                rng = MersenneTwister(19)
                                )
...
function run_experiment_greedy!(current_sim_obj::NavigationSimulator{VehicleParametersLSPlanner}, path_planning_details::PathPlanningDetails,
                                                pomdp_details::POMPDPlanningDetails, exp_details::ExperimentDetails, 
                                                shield_utils, output::Output,
                                                sudden_break_flag = true,
                                                run_shield = false,
    ...
    while(!stop_simulation!(current_sim_obj,current_time_value,exp_details,output))
        ...
        # Check if the predicted vehicle position is in the goal region
        # true_goal = exp_details.true_target
        if(is_within_range(predicted_vehicle_center_x,predicted_vehicle_center_y,exp_details.true_target.x,
            exp_details.true_target.y,exp_details.radius_around_vehicle_goal))
            println("Predicted vehicle state is in goal. Experiment Finished")
            next_pomdp_action = ActionLimitedSpacePOMDP(0.0)
            output.nearby_humans[current_time_value] = nbh
            output.b_root[current_time_value] = nothing
        ...
        # If it is not, then take observation from the environment and plan for the next action
            ...
            next_pomdp_action, info = custom_action_info(predicted_vehicle_center_x, predicted_vehicle_center_y,
                                        predicted_vehicle_state.v, nbh, 2.0)
...
function custom_action_info(vehicle_x, vehicle_y, vehicle_v, nbh, threshold=2.0)
    
    # Check for humans too close to the vehicle
    too_close_humans = false
    for i in 1:length(nbh.position_data)
        human_x = nbh.position_data[i].x
        human_y = nbh.position_data[i].y
        
        # Calculate distance between vehicle and human
        distance = sqrt((vehicle_x - human_x)^2 + (vehicle_y - human_y)^2)
        
        if distance < threshold
            too_close_humans = true
            println("Human $i is too close: $distance m")
            break
        end
    end
    
    if too_close_humans
        next_action = ActionLimitedSpacePOMDP(-0.5)
    else
        if(vehicle_v == 0.0)
            next_action = ActionLimitedSpacePOMDP(0.5)
        else
            next_action = ActionLimitedSpacePOMDP(0.0)
        end
    end
    
    # Create empty info for consistency with the expected return format
