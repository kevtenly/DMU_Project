...
function NewLimitedSpacePOMDP(pomdp_details,env,vehicle_params,rollout_guide,sudden_break_flag,goal=nothing)

    if(goal == nothing)
        return LimitedSpacePOMDP(pomdp_details,env,vehicle_params,rollout_guide,sudden_break_flag)
    else
        return LimitedSpacePOMDP(
            pomdp_details.discount_factor,
            pomdp_details.min_safe_distance_from_human,
...
function POMDPs.gen(m::LimitedSpacePOMDP, s, a, rng)
    ...
    sp = StateLimitedSpacePOMDP(new_vehicle_position[1],new_vehicle_position[2],new_vehicle_position[3],new_vehicle_speed,
                                    s.index_vehicle_controls_sequence+num_path_steps,next_human_states)
    ...
    for (s, w) in weighted_particles(b)
        if(s.vehicle_x == -100.0 && s.vehicle_y == -100.0)
            ...
            elseif(is_within_range(vehicle_center_x,vehicle_center_y, m.vehicle_goal.x, m.vehicle_goal.y,
                ...
                value_sum += w*((discount(m)^time_to_goal(m,s))*m.goal_reached_reward)
                if(debug)
...
function get_default_action(m::LimitedSpacePOMDP,b,ex)
    # println("Default Action is Triggered")
    if(m.sudden_break_flag)
        return ActionLimitedSpacePOMDP(-10.0)
    else
