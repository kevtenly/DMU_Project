...
aspen
aspen2function wanderer!(st::TargetState, dt::Float64; ω_std =0.4, rng::AbstractRNG = Random.GLOBAL_RNG, env_length::Float64 = Inf, env_breadth::Float64 = Inf)
@info "enter wanderer!" (st.x, st.y)
ω = randn(rng) * ω_std
dubins_step!(st, ω, dt)

if st.x <= 0 || st.x >= env_length
st.theta = π - st.theta
end:POMPDPlanningDetails, exp_details::ExperimentDetails, 
...
if st.y <= 0 || st.y >= env_breadth
st.theta = - st.theta
end

st.x = clamp(st.x, 0.0, env_length)
st.y = clamp(st.y, 0.0, env_breadth)

@info "exit wanderer" (st.x, st.y)
return nothing
end
...
veh_goal = Location(exp_details.target_state.x, exp_details.target_state.y)
#Replace veh_goal with mean of initial belief
...
vehicle_controls_sequence = hybrid_astar_search(env,veh,temp_veh_params,vehicle_steer_actions,nbh,path_planning_details);
...
run_experiment_moving_target!(initial_sim_obj, path_planning_details, pomdp_details, exp_details, 
                shield_utils, output, SB_flag, RS_flag, run_with_trials)
